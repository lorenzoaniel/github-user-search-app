...
interface Props {
	variant: string;
	linkData: string | null | undefined; //from api
}

const Links: React.FC<Props> = ({ variant, linkData = null }) => {
	const linkImgSrcVariant: { [key: string]: any } = {
		location: (
			<svg className=" min-w-fit" height="20" width="14" xmlns="http://www.w3.org/2000/svg">
				<path
					className={`dark:fill-white ${linkData ?? "opacity-50"}`}
		...
		blog: (
			<svg className=" min-w-fit" height="20" width="20" xmlns="http://www.w3.org/2000/svg">
				<g fill="#4b6a9b">
					<path
						className={`dark:fill-white ${linkData ?? "opacity-50"}`}
	...
	return (
      ...
      "
		>
			{linkImgSrcVariant[variant]}
			{variant === "location" ? (
				<span
					className={`links-wrapper-linkcomp
          w-fit
          h-fit
          text-h4
					truncate
